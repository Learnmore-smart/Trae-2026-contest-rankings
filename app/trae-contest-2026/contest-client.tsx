...
type RingTone = "cyan" | "green" | "amber" | "violet";
...
export function NavMenu({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div
        className={`dropdown-menu absolute right-0 z-50 mt-2 w-48 rounded-md p-2 backdrop-blur-xl transition ${
          open ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-1 opacity-0"
        }`}
        role="menu"
      >
...
const PHASE_PROGRESS: Record<RunPhase, number> = { idle: 0, scrape: 33, match: 66, judge: 90, done: 100, error: 100 };
...
function RunButton({ language, onCompleted }: { language: ContestLanguage; onCompleted: () => void }) {
  ...
  const startPolling = useCallback(() => {
    ...
    pollRef.current = window.setInterval(async () => {
      try {
        const response = await fetch("/api/trae-contest/run", { cache: "no-store" });
  ...
  const trigger = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/trae-contest/run", { method: "POST" });
      const next = (await response.json()) as PipelineStatus;
      setStatus(next);
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      {!running && (phase === "done" || phase === "error" || cooldown) ? (
        <span className={`pipeline-status ${phase === "error" ? "is-error" : phase === "done" ? "is-done" : ""}`}>
          <span>{cooldown ? t.cooldown : phaseMessage(phase, language)}</span>
        </span>
      ) : null}
    </div>
  );
        ...
        {OFFICERS.map((officer) => (
          <article key={officer.src} className="officer-card">
            <div className="officer-frame">
              ...
              <img className="officer-portrait" src={officer.src} alt={`${officer.name[language]} · ${t.officers}`} loading="lazy" />
            </div>
...
function RankCard({ item, language, viewMode }: { item: RankingItem; language: ContestLanguage; viewMode: ViewMode }) {
  ...
  const detailHref = `/trae-contest-2026/project/${encodeURIComponent(item.topic.id)}`;
  ...
  return (
    ...
    >
      ...
      <div className="rank-row__scores" aria-label={t.totalScore}>
        <ScoreRing label={t.totalScore} value={item.evaluation?.totalScore} max={100} language={language} tone={tier === "gold" ? "amber" : "cyan"} size="large" />
      ...
      <nav className="site-nav">
        <div className="site-nav__group">
          <MainTabs
            activeTab={activeTab}
            items={[
              { key: "landing" as const, label: t.navLanding, href: "/trae-contest-2026" },
              { key: "ranking" as const, label: t.navRanking, href: "/trae-contest-2026/ranking" }
            ]}
          />
              ...
              <div className="hero-command-deck">
                ...
                <div className="landing-hero-side">
                  ...
                  <div className="hero-actions">
                    <Link href="/trae-contest-2026/ranking" className="control-button primary">
